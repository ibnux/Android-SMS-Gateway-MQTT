...
public class LogAdapter extends RecyclerView.Adapter<LogAdapter.MyViewHolder> {
    ...
    public void reload() {
        ...
        bigTime = 0;
        if (search.length() > 0) {
            datas = ObjectBox.get().boxFor(LogLine.class).query().contains(LogLine_.message, search, QueryBuilder.StringOrder.CASE_INSENSITIVE).orderDesc(LogLine_.time).build().find(offset, limit);
        } else {
    ...
    public void getNewData() {
        List<LogLine> dts;
        if (search.length() > 0) {
            dts = ObjectBox.get().boxFor(LogLine.class).query().contains(LogLine_.message, search, QueryBuilder.StringOrder.CASE_INSENSITIVE).greater(LogLine_.time, bigTime).order(LogLine_.time).build().find(offset, limit);
        } else {
    ...
    public void nextData() {
        List<LogLine> dts;
        if (search.length() > 0) {
            dts = ObjectBox.get().boxFor(LogLine.class).query().contains(LogLine_.message, search, QueryBuilder.StringOrder.CASE_INSENSITIVE).less(LogLine_.time, smallTime).orderDesc(LogLine_.time).build().find(offset, limit);
        } else {
